...
const WeeklyOffScheduleConfirmModal: React.FC<Props> = ({ isOpen, onContinue, onChangeDate }) => {
  useEffect(() => {
    if (isOpen) {
      ...
      console.log('[Diagnostic] Popup Opened', { popup: 'WeeklyOffScheduleConfirmModal' });
      return () => {
        console.log('[Diagnostic] Popup Closed', { popup: 'WeeklyOffScheduleConfirmModal' });
  ...
  const modalNode = (
    <AnimatePresence>
      {isOpen ? (
        <div className="fixed inset-0 z-[10350] flex items-center justify-center bg-gray-900/60 p-4 backdrop-blur-sm">
          <motion.div
