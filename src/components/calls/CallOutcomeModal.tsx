...
import { fetchGroupedSubstages, GroupedSubstages } from '../../services/substages.api';
...
export const CallOutcomeModal: React.FC<CallOutcomeModalProps> = ({
  ...
  // Dynamic Query for active Lead Stages and Substages from Master Configuration
  const {
    data: groupedSubstages = [],
    isLoading: loadingSubstages,
    isError: isLoadError,
    error: loadError,
    refetch: refetchStages,
  } = useQuery<GroupedSubstages[]>({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      void queryClient.invalidateQueries({ queryKey: ['grouped-substages'] });
    }
  }, [isOpen, queryClient]);

  if (!isOpen) return null;

  // Determine if current stage is configured as LOB
  const currentStageGroup = groupedSubstages.find(
    (g) => g.name.trim().toLowerCase() === (currentStageName || '').trim().toLowerCase(),
  );
  const isCurrentLOB = Boolean(
    currentStageGroup?.isLOB || (currentStageName || '').toLowerCase().includes('lob'),
  );
  ...
  const leadItemForDrawer = {
    ...
    name: leadName,
    phone: leadPhone,
    stage: currentStageName ? { id: currentStageGroup?.id || '', name: currentStageName } : undefined,
    substage: currentSubstageName ? { id: '', name: currentSubstageName } : undefined,
  } as unknown as LeadListItem;

  // Find target stage configuration based on selection
  const resolveTargetGroup = (): GroupedSubstages | null => {
    if (selectedSubstageId) {
      return groupedSubstages.find((g) => g.substages.some((sub) => sub.id === selectedSubstageId)) || null;
    }
    if (selectedStageId) {
      return groupedSubstages.find((g) => g.id === selectedStageId) || null;
    }
    return null;
  };

  const handleSubstageClick = (substageId: string, stageId: string) => {
    if (selectedSubstageId === substageId) {
      setSelectedSubstageId(null);
      setSelectedStageId(null);
    } else {
  ...
  const handleDirectStageClick = (stageId: string) => {
    if (selectedStageId === stageId && !selectedSubstageId) {
      setSelectedStageId(null);
    } else {
      setSelectedSubstageId(null);
      setSelectedStageId(stageId);
  ...
  const executeOutcomeSubmission = async (extraPayload?: {
    ...
    const payload: SaveCallOutcomePayload = {
      ...
      targetStageId: targetGroup?.id || selectedStageId || undefined,
          ...
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6 flex-1 text-gray-800">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                {(selectedSubstageId || selectedStageId) && (
                  <button
                    type="button"
              ...
              {loadingSubstages ? (
                <div className="p-8 text-center bg-gray-50/80 rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-2">
                  <div className="w-5 h-5 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />
                  <span className="text-gray-500 text-xs font-bold">Loading configured lead stages &amp; substages...</span>
                </div>
              ) : isLoadError ? (
                <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-semibold flex items-center justify-between shadow-sm">
                  <span>Failed to load lead stages: {(loadError as any)?.message || 'Unknown error'}</span>
                  <button
                    type="button"
                    onClick={() => refetchStages()}
                    className="inline-flex items-center gap-1 px-3 py-1 bg-white border border-rose-300 rounded-lg text-rose-700 font-bold hover:bg-rose-100 text-[11px] cursor-pointer"
                  >
                ...
                <div className="p-6 rounded-2xl bg-gray-50 border border-gray-200 text-center text-xs text-gray-500 font-semibold">
                  No active Lead Stages are configured in Master Configuration.
                </div>
              ) : (
                <div className="space-y-3">
                  {groupedSubstages.map((stageGroup) => {
                    ...
                    return (
                      <div
                        key={stageGroup.id}
                        className="p-4 rounded-2xl bg-gray-50/80 border border-gray-200/60"
                      >
                        ...
                        {filteredSubstages.length > 0 ? (
                            ...
                            {filteredSubstages.map((sub) => {
                              ...
                              return (
                                <button
                                  key={sub.id}
                                  type="button"
                                  onClick={() => handleSubstageClick(sub.id, stageGroup.id)}
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => handleDirectStageClick(stageGroup.id)}
                              className={`px-3.5 py-2 rounded-xl text-xs font-extrabold border transition-all flex items-center gap-1.5 cursor-pointer ${
                            ...
                            >
                              <Tag className="w-3.5 h-3.5 opacity-70" />
                              <span>Select {stageGroup.name} Stage</span>
                            </button>
                            <span className="text-[11px] text-gray-400 font-semibold italic">
                              (No substages configured)
                            </span>
                          </div>
                        )}
                      </div>
