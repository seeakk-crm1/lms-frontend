...
const QuickLeadWidget = () => {
    ...
    const handleSave = (e) => {
        ...
        // Mock save
        const toastId = toast.loading("Saving new lead...");
        setTimeout(() => {
            toast.success(`${name} added to New Leads`, { id: toastId });
            setName('');
            setPhone('');
        }, 800);
    };
        ...
        >
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4 flex-1">
                ...
                <div className="mt-auto pt-4">
                    <button
                        type="submit"
                        className="w-full bg-gray-900 hover:bg-black text-white py-2.5 rounded-xl text-sm font-bold shadow-sm active:scale-95 transition-all text-center"
                    >
                        Save Lead
                    </button>
                </div>
