...
import LOBReasonForm from './LOBReasonForm';
import type { LOBReason, LOBReasonPayload } from '../types/lobReason.types';

interface LOBReasonModalProps {
  open: boolean;
  onClose: () => void;
  initialValue?: LOBReason | null;
  onSubmit: (payload: LOBReasonPayload) => Promise<void>;
  isSubmitting?: boolean;
}

const LOBReasonModal: React.FC<LOBReasonModalProps> = ({ open, onClose, initialValue, onSubmit, isSubmitting }) => (
  <AnimatePresence>
    {open ? (
      <motion.div
        initial={{ opacity: 0 }}
          ...
          <div className="flex items-start justify-between gap-4 border-b border-gray-100 px-4 py-4 sm:px-8 sm:py-5">
            <div className="min-w-0">
              <div className="mb-2 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-emerald-600">
                {initialValue ? 'Edit LOB Reason' : 'Create LOB Reason'}
              </div>
              <h2 className="text-xl font-black tracking-tight text-gray-900 sm:text-2xl">
                {initialValue ? 'Update LOB reason' : 'Add LOB reason'}
              </h2>
            ...
            <button
              ...
              aria-label="Close LOB reason modal"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
